...
CAPABILITIES = sorted({c for _, _, caps in PROVIDERS.values() for c in caps.split(",")} | {"evaluation", "monitoring"})

def _policy_path() -> Path: return APP_DIR / "ecosystem_policy.json"
def _policy() -> dict:
    p = _policy_path()
    if p.exists():
...
@tool("ecosystem_catalog", "ecosystem", "List normalized AI providers, capabilities and credential status", {"capability":"optional"})
def ecosystem_catalog(capability: Optional[str] = None) -> Dict[str, Any]:
    rows=[]
    for pid,(name,key,caps) in PROVIDERS.items():
        values=caps.split(",")
        if capability and capability not in values: continue
...
def ecosystem_route(capability: str, preferred: Optional[str] = None) -> Dict[str, Any]:
    ...
    if not candidates: return {"ok":False,"capability":capability,"error":"no registered provider capability"}
    if preferred and preferred in candidates: candidates=[preferred]+[p for p in candidates if p!=preferred]
...
def ecosystem_policy(action: str = "show") -> Dict[str, Any]:
    ...
    elif action!="show": return {"ok":False,"error":"action must be show|enforce|hybrid"}
    if action!="show":
        _policy_path().parent.mkdir(parents=True,exist_ok=True); _policy_path().write_text(json.dumps(data,indent=2),encoding="utf-8")
    return data
...
def ecosystem_contract() -> Dict[str, Any]:
    return {"name":"AWEAI Universal AI Tool Contract","version":"1.0","operations":["discover","authorize","route","execute","stream","cancel","retry","evaluate","audit","cache","trace"],"input":["capability","task","payload","constraints","preferred_provider","budget","latency"],"output":["provider","model","result","usage","latency","trace_id","warnings","audit_id"],"safety":["explicit_credentials","least_privilege","no_secret_logging","audit_trail","fallback","dry_run"]}

@tool("ecosystem_execute", "ecosystem", "Execute a normalized chat/reasoning/code request through AWEAI-managed adapters", {"capability":"chat|reasoning|code","message":"required","provider":"optional","model":"optional","dry_run":"optional"})
def ecosystem_execute(capability: str, message: str, provider: Optional[str] = None, model: Optional[str] = None, dry_run: bool = False) -> Dict[str, Any]:
    if capability not in ("chat","reasoning","code"):
        return {"ok":False,"error":"execution adapter currently supports chat/reasoning/code; other capabilities remain discoverable until their adapter is installed"}
    route=ecosystem_route(capability, provider)
    selected=route["selected"]
    if dry_run: return {"ok":True,"dry_run":True,"gateway":"AWEAI","route":route,"message":message}
    started=time.time()
    try:
